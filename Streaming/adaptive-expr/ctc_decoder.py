...
class GreedyCTCDecoder(CTCDecoder):
    ...
    def __call__(self, logits, frame_start: int, frame_end: int) -> tuple[list[int], list[float], list[int]]:
        ...
        current_max_frame   = -1 # The frame with the highest peak gets assigned as the phoneme's frame
        prev                = None

        for i, (tid, prob) in enumerate(zip(ids.tolist(), max_probs.tolist())):
            if tid != prev:
                # Append previous phoneme and avrg. its probabilities
...
class BeamEntry:
    ...
    * Used to get avg. over last tokens (a single token can span multiple frames) *
    token_probs : probabilities of the token
    token_counts : number of times this token has repeated consecutively
    token_frames : index of the frame where the token was first observed
    token_max_pc : max probability of the token
    """
...
class BeamCTCDecoder(CTCDecoder):
    ...
    def __call__(self, logits, frame_start: int, frame_end: int) -> tuple[list[int], list[float], list[int]]:
        ...
        # T: number of time steps, V: vocabulary size
        T, V = log_probs.shape
...
# Implemented as an alternative to torch.logaddexp to avoid the overhead of tensors
...
# Helper function to debug results of both
def print_comparison(greedy_result, beam_result):
    ...
    print("="*70)
    return beam_result  # Return beam decoder result
