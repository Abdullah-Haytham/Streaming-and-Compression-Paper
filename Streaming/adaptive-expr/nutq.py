...
# ── Arabic sifat token → English label mapping (mirrors the notebook) ────────
...
class VocabTokenizer:
    ...
    def __init__(self, idx2sym: dict, vocabs: dict):
        ...
        # Pre-calculate English-to-ID dicts to avoid O(N) inversions during inference
        self.sifat_level_to_en_to_id_vocab: dict[str, dict[str, int]] = {
            level: {v: k for k, v in id_to_en.items()}
            for level, id_to_en in self.sifat_level_to_id_to_en_vocab.items()
        }

        # pre-calculate negative Sifat IDs using the newly cached en_to_id dict
        self.sifat_negative_ids: dict[str, int] = {}
        for level, en_to_id in self.sifat_level_to_en_to_id_vocab.items():
            if level in _SIFAT_NEGATIVE_LABEL:
                self.sifat_negative_ids[level] = en_to_id.get(_SIFAT_NEGATIVE_LABEL[level], 0)
    ...
    @property
    def negative_sifat_ids(self) -> dict[str, int]:
        return self.sifat_negative_ids

    def apply_sifat_constraint(self, level: str, base_chars: list[str], aligned_ids: list[int]) -> list[int]:
        if level not in SIFAT_POSITIVE_PHONEMES or level not in _SIFAT_NEGATIVE_LABEL:
            return aligned_ids
            
        neg_id = self.sifat_negative_ids.get(level, 0)
        positive_set = SIFAT_POSITIVE_PHONEMES[level]
        
        for i in range(len(aligned_ids)):
            if base_chars[i] not in positive_set:
                aligned_ids[i] = neg_id
    ...
    def tokenize_refs(self, ref_quran_phonetic_script_list) -> dict[str, list[list[int]]]:
        ...
        for ref in ref_quran_phonetic_script_list:
            ...
            for level in self.level_to_id_vocab:
                ...
                # We can do this safely using the pre-cached en_to_id dict
                en_to_id = self.sifat_level_to_en_to_id_vocab[level]
                sifat_ids = []
                for s in ref.sifat:
